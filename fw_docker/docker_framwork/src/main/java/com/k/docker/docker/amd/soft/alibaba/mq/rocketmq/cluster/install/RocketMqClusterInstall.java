package com.k.docker.docker.amd.soft.alibaba.mq.rocketmq.cluster.install;
...
public class RocketMqClusterInstall extends DockerBaseImpl {
    public RocketMqClusterInstall() {
    ...
    public static void dockerRun(final Class clazz, final List<String> lines, final String dockerVersion, final DockerRunBase dockerRun, final RocketmqClusterHost hostEntity, String param) {
        List<String> hosts;
    ...
    private void buildConsole(final File file, final DockerBuildVO dockerBuildVO, final String compareParam) throws Exception {
        if (dockerBuildVO.tar.contains(".war")) {
            ...
            final String mvDirc = "tomcat/webapps/ROOT.war";
            ...
            ShellWriteUtil.writeCp(file, dockerBuildVO.tar, mvDirc);
    ...
    private void nameBroker(final File file, final DockerBuildVO dockerBuildVO, final String compareParam, final String brockProps) throws Exception {
        ...
        lines.add("nohup ./mqbroker -c $ROCKETMQ_HOME/conf/" + prop + " >$ROCKETMQ_HOME/log/mq.log &");
    ...
    public List<File> buildConfigFile() {
        ...
        try {
            ...
            // {
            // profile = File.createTempFile("config.properties" + tempSub, tempSub);
            // {
            ...
            for (int i = 0; i < masters.size(); i++) {
                {
                    ...
                    buildBrock(files, "Slave", i, 1 + i, ns);
                }
            }
    ...
    private void buildBrock(final List<File> files, final String master_slave, final int index, final int brokerId, final String nameServers) throws Exception {
        ...
        profile = File.createTempFile("mqbroker" + master_slave + +index + ".properties" + DockerBase.tempSub, DockerBase.tempSub);
        {
            ...
            FileWriteUtil.write(profile, "autoCreateTopicEnable=true");
            FileWriteUtil.write(profile, "autoCreateSubscriptionGroup=true");
