package com.k.docker.docker.amd.soft.shadowsocket.install;
...
public class ShadowSocketSingleInstall extends DockerBaseImpl {
    public ShadowSocketSingleInstall() {
    ...
    public ShadowSocketSingleInstall(List<String> hosts) {
        dockerBuildVOs = Lists.newArrayList();
        dockerBuildVOs.add(SoftVersion.rabbitmqRrlangSoft);
        dockerBuildVOs.add(SoftVersion.rabbitmqServerSoft);
        this.hosts = hosts;
    ...
    public static void dockerRun(Class clazz, List<String> lines, String dockerVersion, DockerRunBase dockerRun, RabbitmqHost dockerHost,
                                 String param, Map<String, String> hostMap, List<String> ports) {
        String memery = "600m";
        List<String> hosts;
    ...
    public static void dockerRun(Class clazz, List<String> lines, String dockerVersion, DockerRunBase dockerRun,
                                 RabbitmqClusterHost dockerHost, String param, Map<String, String> hostMap, List<String> ports) {
        String memery = "600m";
        List<String> hosts;
        lines.add("cd " + FWPathUtil.getClassPath(clazz));
        {
    ...
    public List<File> buildConfigFile() {
        ...
        try {
            boolean check = false;
            List<String> lines;
            File config = File.createTempFile("rabbitmq.config" + tempSub, tempSub);
            {
